...
cursor.execute(
    """
    create table if not exists news_stuffs.naver_news_test(
...
for i in range(10)[9:]:
    ...
    date0 = date(2023,1,14) - timedelta(days=i)
    print(date0)
    ...
    for n in range(len(rows['uri'])):
        ...
        if title.strip() == '':
            continue
        createDate = rows['createDate'][n]
        temp = requests.get(uri, headers=headers2)
        temp = BeautifulSoup(temp.content, 'html.parser')
        article = temp.find('div', {'id':'newsct_article'})
        if article == None:
            article = temp.find('div', {'id': 'newsEndContents'})

        for div in article.find_all("div", {'class':'media_end_summary'}):
            div.decompose()
        for div in article.find_all("em", {'class':'img_desc'}):
            div.decompose()

        article = article.text.replace('\n','')
        blob0 = bin(int(binascii.hexlify(article.encode('utf-8')), 16))[2:]
        cursor.execute(
            f"""
            insert into news_stuffs.naver_news_test values(NULL, "{uri}", {cv},{cv_p}, "{title}", "{createDate}", b'{blob0}')
            """
        )
